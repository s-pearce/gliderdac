...
def par_from_volts(par_volts, sensor_dark, scale_factor):
    """PAR in uE/(m^2 sec) from sensor voltage and calibration coefficients

    :param par_volts: array [V]
    :param sensor_dark: float [mV]
        The `Sensor Dark` coefficient from the instrument's relevant
        Calibration Certificate in millivolts.
    :param scale_factor: float [Volts/(uE/(m^2 sec))/ ]
        The `Wet Calibration scale_factor` for units uE/(m^2 sec) from the
        instrument's relevant Calibration Certificate.

    :returns PAR: array [uE/(m^2 sec)]
    """
    return (par_volts - (sensor_dark/1000)) / scale_factor
